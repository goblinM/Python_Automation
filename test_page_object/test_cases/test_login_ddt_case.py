...
Author:goblinM(mmx)
Date:2020-01-15
Describe:业务层(测试用例层) 使用ddt(data driver test) 进行数据驱动测试
"""
import os

from test_page_object.logic_layer.ddt_logic import larger_than_two, has_three_elements, is_a_greeting
import unittest
...
class FooTestCase(unittest.TestCase):
    ...
    @file_data(os.path.join(current_path, "data_layer", "test_data_dict_dict.json"))
    def test_file_data_json_dict_dict(self, start, end, value):
        self.assertLess(start, end)
        self.assertLess(value, end)
        self.assertGreater(value, start)

    @file_data(os.path.join(current_path, "data_layer",'test_data_dict.json'))
    def test_file_data_json_dict(self, value):
        self.assertTrue(has_three_elements(value))

    @file_data(os.path.join(current_path, "data_layer",'test_data_list.json'))
    def test_file_data_json_list(self, value):
        self.assertTrue(is_a_greeting(value))

    @needs_yaml
    @file_data(os.path.join(current_path, "data_layer","test_data_dict_dict.yaml"))
    ...
    @needs_yaml
    @file_data(os.path.join(current_path, "data_layer",'test_data_dict.yaml'))
    def test_file_data_yaml_dict(self, value):
        self.assertTrue(has_three_elements(value))

    @needs_yaml
    @file_data(os.path.join(current_path, "data_layer",'test_data_list.yaml'))
